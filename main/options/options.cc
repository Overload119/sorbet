...
buildOptions(const vector<pipeline::semantic_extension::SemanticExtensionProvider *> &semanticExtensionProviders) {
    ...
    options.add_options("advanced")("autogen-autoloader-packaged", "Generate the packaged autoloader files",
                                    cxxopts::value<bool>());
    options.add_options("advanced")(
        "autogen-autoloader-packaged-shim-paths",
        "Extra directories in autogenerated paths which contain per-package autoload shims. "
        "This option must be used in conjunction with --stripe-packages",
        cxxopts::value<vector<string>>(), "string");
...
bool extractAutoloaderConfig(cxxopts::ParseResult &raw, Options &opts, shared_ptr<spdlog::logger> logger) {
    ...
    if (raw.count("autogen-autoloader-packaged-shim-paths") > 0) {
        if (!opts.stripePackages) {
            logger->error("--autogen-autoloader-packaged-shim-paths can only be specified in --stripe-packages mode");
            throw EarlyReturnWithCode(1);
        }
        for (const string &dirName : raw["autogen-autoloader-packaged-shim-paths"].as<vector<string>>()) {
            cfg.packagedShimPaths.emplace_back(dirName);
        }
    }
